...
function MarketingCloudConnectInner() {
  const searchParams = useSearchParams();
  const initialError = searchParams.get("error");
  const platform = PLATFORM_CONFIG.marketing_cloud;
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch("/api/connect/marketing-cloud", {
      method: "POST",
        ...
        <div className="max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-zinc-400">
              Use an Installed Package with server-to-server API integration. Include MID only when connecting a specific business unit.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input name="authBaseUrl" type="url" required placeholder="Authentication Base URI" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white placeholder-zinc-500 focus:outline-none focus:border-brand" />
            ...
            <input name="accountId" placeholder="Business Unit MID (optional)" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white placeholder-zinc-500 focus:outline-none focus:border-brand" />
            <button
              type="submit"
              disabled={submitting}
...
export default function MarketingCloudConnectPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <MarketingCloudConnectInner />
    </Suspense>
  );
}
